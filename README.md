...
| Path | Purpose |
| --- | --- |
| `skills/codelab-publisher/SKILL.md` | The skill definition and end-to-end workflow. |
| `skills/codelab-publisher/assets/codelab.md.template` | Starter `codelab.md` with the required metadata header. |
| `skills/codelab-publisher/references/` | Deep-dive docs: codelab markdown format, `claat` reference, GitHub Pages deploy options, and the brainstorming question set. |
| `skills/codelab-publisher/scripts/` | Helpers to check prerequisites, install `claat`, and deploy. |
...
### 1. Install the Skill
Copy or symlink this directory into your agent's skills folder depending on your environment, or use `npx skills add`:
```bash
npx skills add https://github.com/gerfalcon/codelab-publisher-skill
```
...
- **`claat` CLI**: Run the installer script to set up the Google Codelab CLI:
  ...
  bash .agents/skills/codelab-publisher/scripts/install_claat.sh
  ```
...
```bash
# If installed at project-level:
bash .agents/skills/codelab-publisher/scripts/check_prereqs.sh
```
...
4. **Feedback Form Generation** — Once the codelab is verified, the skill automatically extracts its step-by-step structure to construct a custom prompt. Copying this prompt directly into Gemini within the Google Forms website instantly builds a matching multi-section feedback Google Form.
